...
const ProfilePage: React.FC = () => {
  ...
  const handleLogout = async () => {
    try {
      ...
      await logout();
      navigate(`${LEADERS_ROOT}/login`, { replace: true });
    } catch (error) {
  ...
  const handleToggleLanguage = async () => {
    ...
    if (user?.uid && (nextLocale === 'es' || nextLocale === 'en')) {
      try {
        await patchUserPreferences(user.uid, { preferredLocale: nextLocale });
        await refreshProfile();
      } catch (e) {
        console.warn('patchUserPreferences preferredLocale:', e);
      }
    }
  };
  ...
  return (
    <PageShell title={t('auth.profile.title')} variant="default">
      <div className="profile-page">
        ...
        <Card variant="default" padding="md" className="profile-card profile-card--badges">
          <p className="profile-email profile-email--hero-meta">{displayEmail}</p>
